...
int main(int argv, char** argc)
{
  ...
  if (argv >= 2) {
    for (int i = 1; i < argv; ++i) {
      ...
      file.open(argc[i]);
      if (file.is_open()) {
        dicts.insert(std::make_pair(name.substr(0, name.find('.')), bazhin::readDictionary(file)));
      } else {
  ...
  bazhin::Commands commands(std::cin, std::cout, dicts);
  ...
  while (std::cin >> command) {
    try {
      ...
      bazhin::printInvalidCommand(std::cout) << "\n";
    }
